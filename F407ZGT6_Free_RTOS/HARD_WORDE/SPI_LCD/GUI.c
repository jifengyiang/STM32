...
void Gui_Drawbmp128(u16 x,u16 y,const unsigned char *p) //显示40*40 QQ图片
{
  	int i; 
	unsigned char picH,picL; 
			  ...
			  { 	LCD_SetWindows(x,y,x+64-1,y+64-1);
				    for(i=0;i<64*8;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(TFONT64[k].Msk[i]&(0x80>>j))	LCD_WR_DATA(fc);
								else LCD_WR_DATA(bc);
							}
							else
							{
...
void GUI_DrawFont128(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	int i,j;
	u16 k;
	u16 HZnum;
	u16 x0=x;
	HZnum=sizeof(TFONT128)/sizeof(typFNT_GB128);	//自动统计汉字数目
	for (k=0;k<HZnum;k++) 
			{
			  if ((TFONT128[k].Index[0]==*(s))&&(TFONT128[k].Index[1]==*(s+1)))
			  { 	LCD_SetWindows(x,y,x+128-1,y+128-1);
				    for(i=0;i<128*16;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(TFONT128[k].Msk[i]&(0x80>>j))	LCD_WR_DATA(fc);
								else LCD_WR_DATA(bc);
							}
							else
							{
								POINT_COLOR=fc;
								if(TFONT128[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//画一个点
								x++;
								if((x-x0)==128)
								{
									x=x0;
